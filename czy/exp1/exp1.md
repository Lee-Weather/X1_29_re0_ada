...
| exp_ada_1.4 | 2026-09-03 | 参考轨迹抬脚充分性（FK 扫描定参）：膝 scale 下限 1.0（抬脚主体保满幅）+髋下限 0.85（步态自然）+**踝摆幅符号反转**（跖屈压脚尖→背屈勾脚，FK: ±8mm 差）；target 回 0.03 基线（奖励不动，改参考指挥）；L20 从零 6000 轮 | 🔄训练中（6000 轮） | 待定 | limxmt8fxevjvx38jd@emalupe.com | 待定 |
